      ...
      <div className='auth-card'>
        <div className='auth-card-box'>
          ...
          <div className='auth-card-top'>
            ...
            <InputBox ref={emailRef} type='text' label='이메일' placeholder='이메일 주소를 입력해주세요.' error={error} value={email} onChange={onEmailChangeHandler} onKeyDown={onEmailKeyDownHandler} />
            <InputBox ref={passwordRef} type={passwordType} label='비밀번호' placeholder='비밀번호를 입력하세요.' error={error} value={password} onChange={onPasswordChangeHandler} icon={passwordButtonIcon} onButtonClick={onPasswordButtonClickHandler} onKeyDown={onPasswordKeyDownHandler} />
          </div>
  ...
  const SignUpCard = () => {
    ...
    const signUpResponse = (responseBody: SignUpResponseDto | ResponseDto | null) => {
      ...
      if (code === 'DN') {
        ...
        setNicknameErrorMessage('중복되는 닉네임임입니다.');
      }
    ...
    //        event handler: 개인정보동의 체크 박스 클릭 이벤트 처리        //
    ...
    //        event handler: 첫번째 다음으로 버튼 클릭 이벤트 처리       //
    const onFirstNextButtonClickHandler = () => {
    ...
    //        event handler: 두번째 다음으로 버튼 클릭 이벤트 처리        //
    const onSecondNextButtonClickHandler = () => {
    ...
    const onSignUpButtonClickHandler = () => {
      ...
      const requestBody: SignUpRequestDto = {
        email, password, nickname, agreedPersonal
      };
    ...
    const onPasswordCheckKeyDownHandler = (event: KeyboardEvent<HTMLInputElement>) => {
      if (event.key !== 'Enter') return;
      onSecondNextButtonClickHandler();
    }
    //        event handler: 닉네임 확인 인풋 키 다운 이벤트 처리      //
    ...
    return(
      <div className='auth-card'>
        <div className='auth-card-box'>
          ...
          <div className='auth-card-bottom'>
            {page === 1 && (
              <div className='black-large-full-button' onClick={onFirstNextButtonClickHandler}>{'다음 단계'}</div>
            )}

            {page === 2 && (
              <div className='black-large-full-button' onClick={onSecondNextButtonClickHandler}>{'다음 단계'}</div>
            )}
      ...
      <div className='auth-container'>
        <div className='auth-jumbotron-box'>
          <div className='auth-jumbotron-contents'>
            ...
            <div className='auth-jumbotron-text'>{'MY MAP'}</div>
          </div>
        </div>
